...
def generate_gallery_zip(gallery_id):
    ...
    try:
            ...
            for idx, photo in enumerate(valid_photos, 1):
                try:
                    ...
                    if not image_data or len(image_data) == 0:
                        print(f"  ⚠️  Photo {idx}/{len(valid_photos)} ({photo_id}): Empty file, skipping")
                        failed_photos.append(photo_id)
                        continue
        ...
        if failed_photos:
            print(f"⚠️  Failed to add {len(failed_photos)} photos to ZIP (out of {len(valid_photos)} valid photos)")
        ...
        try:
            ...
            if cloudfront_dist_id:
                ...
                try:
                    ...
                    print(f"⚠️  Could not invalidate CloudFront cache: {str(cf_error)}")
        ...
        if failed_photos:
            print(f"⚠️  {len(failed_photos)} photos were skipped (missing S3 files or errors)")
