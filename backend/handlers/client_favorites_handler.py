...
def handle_add_favorite(user, body):
    ...
    try:
        ...
        try:
            ...
            # Need to fetch user object first since get_user_features expects it
            from utils.config import users_table
            photographer_user_resp = users_table.get_item(Key={'email': user_id}) # Wait, user_id is ID not email. users_table key is email?
            # Actually users_table PK is email. 'user_id' is usually the ID.
            # get_user_features handles ID lookup via GSI if needed.
            
            # We construct a minimal user object with ID
...
def handle_submit_favorites(user, body):
    ...
    try:
        ...
        if success:
            ...
            print(f"⚠️ Notification failed for {photographer_email}")
