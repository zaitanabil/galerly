...
import { Mail, Lock, User, Camera, Users, AlertCircle, Check, ArrowLeft, Eye, EyeOff } from 'lucide-react';
  ...
  const handleVerifyAndRegister = async (e: FormEvent) => {
    ...
    try {
      ...
      const registerResponse = await api.post('/auth/register', {
        email: formData.email,
        name: formData.name,  // Send full name, backend will generate username
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  required
                    minLength={8}
                  className="w-full pl-12 pr-12 py-3.5 bg-white/50 border border-gray-200 rounded-2xl text-[#1D1D1F] placeholder-[#1D1D1F]/40 focus:outline-none focus:ring-2 focus:ring-[#0066CC]/20 focus:border-[#0066CC] transition-all"
                  placeholder="••••••••"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-[#1D1D1F]/40 hover:text-[#1D1D1F]/60 transition-colors"
                  tabIndex={-1}
                >
                  {showPassword ? (
