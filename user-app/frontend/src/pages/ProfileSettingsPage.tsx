...
export default function ProfileSettingsPage() {
  ...
  const [watermarkSettings, setWatermarkSettings] = useState({
    ...
    watermark_type: 'text' as 'text' | 'logo',
    watermark_text: '',
    watermark_logo_s3_key: '',
    watermark_logo_preview: '',
    watermark_position: 'bottom-right',
    watermark_opacity: 0.5,
    watermark_size: 10.0  // For logo size as percentage
  });
    ...
    try {
        ...
        if (response.success && response.data) {
            setWatermarkSettings({
                ...
                watermark_type: (response.data.watermark_type as 'text' | 'logo') || 'text',
                watermark_text: response.data.watermark_text || '',
                watermark_logo_s3_key: response.data.watermark_logo_s3_key || '',
                watermark_logo_preview: '',  // Will be set after loading
                watermark_position: response.data.watermark_position || 'bottom-right',
                watermark_opacity: response.data.watermark_opacity ?? 0.5,
                watermark_size: response.data.watermark_size ?? 10.0
            });
            
            // Load logo preview if exists
            if (response.data.watermark_logo_s3_key) {
                loadLogoPreview(response.data.watermark_logo_s3_key);
            }
        }
    } catch {
  ...
  const loadLogoPreview = async (s3Key: string) => {
    try {
        // Generate CDN URL for preview
        const cdnUrl = `${import.meta.env.VITE_CDN_URL || 'https://cdn.galerly.com'}/${s3Key}`;
        setWatermarkSettings(prev => ({ ...prev, watermark_logo_preview: cdnUrl }));
    } catch (error) {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
        ...
        reader.onload = async (event) => {
            ...
            // Upload to backend
            const uploadResponse = await api.post('/profile/watermark-logo', {
                file_data: base64Data,
                filename: file.name
            });

            if (uploadResponse.success && uploadResponse.data?.s3_key) {
                setWatermarkSettings(prev => ({
                    ...prev,
                    watermark_logo_s3_key: uploadResponse.data.s3_key,
                    watermark_logo_preview: base64Data,
                    watermark_type: 'logo'
                }));
                toast.success('Logo uploaded successfully');
            } else {
                toast.error(uploadResponse.error || 'Failed to upload logo');
            }
  ...
  const handleDeleteAccount = async () => {
    if (deleteConfirmationText.toLowerCase() !== 'delete my account') {
      await showAlert('Invalid Confirmation', 'Please type "delete my account" to confirm.', 'error');
      return;
    }

    const confirmed = await showConfirm(
      'Delete Account',
      'Are you absolutely sure? This action cannot be undone.\n\nAll your galleries, photos, and data will be permanently deleted.\n\nThis process may take a few minutes to complete.',
      'Delete My Account',
      'Cancel',
      'danger'
    );
    ...
    try {
      ...
      if (response.success) {
        ...
        await showAlert(
          ...
          'Your account deletion is in progress. You will be logged out now.\n\nAll your data will be permanently deleted within a few minutes.', 
          'success'
        );
        ...
        await showAlert('Error', response.error || 'Failed to delete account', 'error');
      }
    } catch (error) {
      console.error('Error deleting account:', error);
      await showAlert('Error', 'An error occurred while deleting your account.', 'error');
    }
  };
            ...
            {(user?.plan !== 'free') && (
                <div className="border-t border-gray-200 pt-8">
                    ...
                    <div className="space-y-6">
                        ...
                        <div>
                            <CustomDomainConfig
                                initialDomain={portfolioSettings.custom_domain}
                                onUpdate={(domain) => setPortfolioSettings({ ...portfolioSettings, custom_domain: domain })}
                                    />
                                </div>
                        ...
                        <div className="mt-8">
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <h3 className="text-sm font-medium text-[#1D1D1F]">White Label Branding</h3>
                            </div>
                            <WhiteLabelConfig />
                        </div>
                    ...
                    <div className="space-y-6">
                        ...
                        {watermarkSettings.watermark_enabled && (
                            <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-[#1D1D1F] mb-2">Watermark Type</label>
                                    <div className="inline-flex rounded-xl bg-gray-100 p-1">
                                        <button
                                            type="button"
                                            onClick={() => setWatermarkSettings({ ...watermarkSettings, watermark_type: 'text' })}
                                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                                                watermarkSettings.watermark_type === 'text'
                                        ...
                                        <button
                                            type="button"
                                            onClick={() => setWatermarkSettings({ ...watermarkSettings, watermark_type: 'logo' })}
                                            className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                                                watermarkSettings.watermark_type === 'logo'
                                                    ? 'bg-white text-[#1D1D1F] shadow-sm'
                                                    : 'text-[#1D1D1F]/60 hover:text-[#1D1D1F]'
                                            }`}
                                        >
                                ...
                                {watermarkSettings.watermark_type === 'logo' && (
                                    <div className="space-y-4">
                                        <div>
                                            <label className="block text-sm font-medium text-[#1D1D1F] mb-2">Watermark Logo</label>
                                            
                                            {/* Logo Preview */}
                                            {watermarkSettings.watermark_logo_preview && (
                                                <div className="mb-3 p-4 bg-gray-50 rounded-xl border border-gray-200">
                                                    <img 
                                                        src={watermarkSettings.watermark_logo_preview} 
                                            ...
                                            {/* Upload Button */}
                                            <label className="flex items-center justify-center gap-2 px-4 py-3 bg-white border border-gray-200 rounded-xl text-sm font-medium text-[#1D1D1F] hover:bg-gray-50 transition-all cursor-pointer">
                                                <input
                                                    type="file"
                                                    accept="image/*"
                                                ...
                                                {uploadingLogo ? (
                                                    ...
                                                    <>
                                                        ...
                                                        <span>{watermarkSettings.watermark_logo_s3_key ? 'Change Logo' : 'Upload Logo'}</span>
                                                    </>
                                                )}
                                            </label>
                                            <p className="mt-2 text-xs text-gray-500">
                                                PNG recommended. Max 2MB. Transparent background works best.
                                            </p>
                                        </div>
                                        ...
                                        <div>
                                            <label className="block text-sm font-medium text-[#1D1D1F] mb-2">
                                                Logo Size ({Math.round(watermarkSettings.watermark_size)}% of image)
                                            </label>
                                            <input
                                                type="range"
                                                min="5"
                                                max="30"
                                                step="1"
                                                value={watermarkSettings.watermark_size}
                                                onChange={(e) => setWatermarkSettings({ ...watermarkSettings, watermark_size: parseFloat(e.target.value) })}
