...
import { Check, AlertCircle, Zap, CreditCard, Download, Settings, LogOut, Minus, HelpCircle, Cloud, Image, ChevronRight, FileText, ShieldCheck, ExternalLink, ArrowRight, Video, Headphones } from 'lucide-react';
...
export default function BillingPage() {
  ...
  const handleDownloadInvoice = async (invoice: Invoice) => {
    try {
      ...
      // Otherwise, fetch from backend using stripe_invoice_id
      if (invoice.stripe_invoice_id) {
        const response = await billingService.getInvoicePdf(invoice.stripe_invoice_id);
        if (response.success && response.data?.pdf_url) {
          window.open(response.data.pdf_url, '_blank');
        } else {
          setNotification({ 
            message: 'Invoice PDF not available yet. Please try again later.', 
            type: 'error' 
          });
        }
      } else {
  ...
  const handleUpdatePaymentMethod = async () => {
    try {
      ...
      if (response.success && response.data?.url) {
        ...
        setNotification({ 
          message: response.error || 'Failed to open payment portal', 
          type: 'error' 
        });
      }
    } catch (error) {
  ...
  // Video usage stats
  const videoLimit = usage?.video_limit?.limit_minutes ?? 0;
  const videoUsed = usage?.video_limit?.used_minutes ?? 0;
  const videoPercent = usage?.video_limit?.usage_percent ?? 0;
  const videoQuality = usage?.video_limit?.quality ?? 'none';
  const videoCount = usage?.video_limit?.video_count ?? 0;
        ...
        {loading ? (
          ...
          <div className="space-y-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
              {/* Membership Card */}
              <div className="lg:col-span-2 relative overflow-hidden rounded-2xl md:rounded-[32px] bg-[#1D1D1F] text-white p-6 md:p-10 shadow-2xl transition-transform duration-500 hover:scale-[1.005] group">
                ...
                <div className="relative z-10 flex flex-col h-full justify-between">
                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 mb-8 md:mb-12">
                    <div className="flex-1">
                      <div className="flex flex-wrap items-center gap-2 mb-3">
                        <span className={`inline-flex items-center px-2.5 py-1 rounded-full backdrop-blur-md border text-xs font-medium tracking-wide ${
                          ...
                          {subscription?.status === 'active' ? 'Active Membership' : subscription?.status === 'canceled' ? 'Canceled' : subscription?.status}
                        </span>
                        {subscription?.subscription?.cancel_at_period_end && subscription?.subscription?.current_period_end && (() => {
                          ...
                          return (
                            <span className="inline-flex items-center px-2.5 py-1 rounded-full bg-amber-500/20 backdrop-blur-md border border-amber-500/30 text-xs font-medium text-amber-200">
                              {daysRemaining > 0 
                      ...
                      {/* Unlimited Photos Indicator */}
                      {/* Plan Benefits */}
                      <div className="mt-3 md:mt-4 flex flex-wrap items-center gap-2">
                        <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/10 backdrop-blur-sm border border-white/10 text-xs text-white/80">
                          <Image className="w-3.5 h-3.5" />
                          <span>Unlimited Photos</span>
                        </div>
                        {/* Support Level Badge */}
                        {subscription?.plan && subscription?.plan !== 'free' && (
                          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-amber-500/20 backdrop-blur-sm border border-amber-500/30 text-xs text-amber-200">
                    ...
                    <div className="text-left sm:text-right">
                       {(PLANS.find(p => p.id === subscription?.plan)?.price.monthly || 0) > 0 ? (
                          <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl md:rounded-2xl px-4 py-3 md:px-6 md:py-4 text-center inline-block">
                             <div className="text-2xl md:text-3xl font-bold text-white tracking-tight">
                                {subscription?.subscription?.interval === 'annual' ? (
                                  <>
                                    ${(PLANS.find(p => p.id === subscription?.plan)?.price.annual || 0) * 12}
                                  </>
                                ) : (
                                  <>
                             ...
                             <div className="text-xs text-white/50 font-medium uppercase tracking-wider">
                               {subscription?.subscription?.interval === 'annual' ? 'Per Year' : 'Per Month'}
                             </div>
                          </div>
                       ) : (
                          <div className="bg-white/10 backdrop-blur-md border border-white/10 rounded-xl md:rounded-2xl px-4 py-3 md:px-6 md:py-4 text-center inline-block">
                             <div className="text-2xl md:text-3xl font-bold text-white tracking-tight">Free</div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 md:gap-12">
                    <div>
                      <div className="flex justify-between items-end mb-3">
                        <div className="flex items-center gap-2 text-white/80">
                          <Cloud className="w-4 h-4 md:w-5 md:h-5 text-blue-400" />
                          <span className="font-medium text-sm md:text-base">Cloud Storage</span>
                        </div>
                    ...
                    <div>
                      <div className="flex justify-between items-end mb-3">
                        <div className="flex items-center gap-2 text-white/80">
                          ...
                          <span className="font-medium text-sm md:text-base">Active Galleries</span>
                        </div>
                        <span className="text-xs md:text-sm font-medium text-white/60">
                          {galleriesLimit === -1 
                    ...
                    {/* Video Usage (if applicable) */}
                    {videoQuality !== 'none' && videoLimit > 0 && (
                      <div className="sm:col-span-2">
                        <div className="flex justify-between items-end mb-3">
                          <div className="flex items-center gap-2 text-white/80">
                            <Video className="w-4 h-4 md:w-5 md:h-5 text-green-400" />
                            <span className="font-medium text-sm md:text-base">Video Usage</span>
                            <span className="text-xs text-white/50 ml-2">
                              ({videoQuality.toUpperCase()}) · {videoCount} {videoCount === 1 ? 'video' : 'videos'}
                            </span>
                          </div>
                          <span className="text-xs md:text-sm font-medium text-white/60">
                            {videoLimit === -1 
                              ? `${videoUsed.toFixed(1)} min Used` 
                              : `${videoUsed.toFixed(1)} / ${videoLimit} min`}
                          </span>
                        </div>
                        <div className="h-2 bg-white/10 rounded-full overflow-hidden backdrop-blur-sm">
                          <div 
                ...
                <div>
                   ...
                   <div className="flex flex-col items-center text-center space-y-4 mb-8">
                      {subscription?.subscription?.payment_method ? (
                        <>
                          <div className="w-16 h-16 bg-gradient-to-br from-blue-500 to-blue-600 rounded-2xl flex items-center justify-center mb-2 shadow-lg">
                             <CreditCard className="w-8 h-8 text-white" strokeWidth={1.5} />
                          </div>
                          <div>
                             <h4 className="font-semibold text-[#1D1D1F] mb-1">
                               {subscription.subscription.payment_method.brand?.toUpperCase() || 'Card'} •••• {subscription.subscription.payment_method.last4}
                             </h4>
                             <p className="text-sm text-[#1D1D1F]/60">
                               Expires {subscription.subscription.payment_method.exp_month?.toString().padStart(2, '0')}/{subscription.subscription.payment_method.exp_year}
                             </p>
                          </div>
                ...
                <div className="space-y-3">
                   ...
                   {subscription?.status === 'active' && subscription?.plan !== 'free' && (
                     <button 
                       ...
                       className="w-full py-3 rounded-xl bg-[#1D1D1F] text-white text-sm font-medium hover:bg-black transition-all flex items-center justify-center gap-2 group/manage shadow-md"
                     >
                   ...
                   {subscription?.status === 'active' && !subscription?.subscription?.cancel_at_period_end && subscription?.plan !== 'free' && (
                     <button
            ...
            <div className="glass-panel p-4 md:p-8">
               <div className="flex items-center justify-between mb-6">
                  <h2 className="text-lg md:text-xl font-medium text-[#1D1D1F]">Billing History</h2>
                  <button className="text-xs md:text-sm font-medium text-[#0066CC] hover:underline flex items-center gap-1">
                 ...
                 <>
                    ...
                    <div className="md:hidden space-y-3">
                       {invoices.map((invoice) => (
                          <div key={invoice.id} className="bg-white border border-gray-100 rounded-xl p-4 hover:shadow-md transition-shadow">
                             ...
                             <button
                                ...
                                disabled={!invoice.invoice_pdf && !invoice.stripe_invoice_id}
                                className="w-full flex items-center justify-center gap-2 px-4 py-2.5 bg-[#0066CC]/5 text-[#0066CC] hover:bg-[#0066CC]/10 rounded-lg transition-all font-medium text-sm disabled:opacity-30 disabled:cursor-not-allowed"
                                title={invoice.invoice_pdf || invoice.stripe_invoice_id ? 'Download Invoice' : 'Invoice not available'}
                             >
                                <Download className="w-4 h-4" />
                    ...
                    {/* Desktop Table View */}
                    <div className="hidden md:block border border-gray-100 rounded-2xl overflow-hidden bg-white">
                                ...
                                <tr key={invoice.id} className="group hover:bg-blue-50/20 transition-colors">
                                   ...
                                   <td className="py-4 px-6 text-right">
                                      <button
                                         onClick={() => handleDownloadInvoice(invoice)}
                                         disabled={!invoice.invoice_pdf && !invoice.stripe_invoice_id}
                                         className="inline-flex items-center justify-center p-2 text-gray-400 hover:text-[#0066CC] hover:bg-blue-50 rounded-lg transition-all disabled:opacity-30 disabled:cursor-not-allowed"
                                         title={invoice.invoice_pdf || invoice.stripe_invoice_id ? 'Download Invoice' : 'Invoice not available'}
                                      >
                                         <Download className="w-4 h-4" />
                                      </button>
