...
export async function confirmUpload(photoId: string, galleryId: string, s3Key: string, filename: string, fileSize: number, fileHash?: string, originalS3Key?: string, originalFilename?: string) {
...
export async function completeMultipartUpload(galleryId: string, uploadId: string, photoId: string, parts: { PartNumber: number; ETag: string }[], fileHash?: string, s3Key?: string, filename?: string) {
  return api.post(`/galleries/${galleryId}/photos/multipart-upload/complete`, {
    ...
    file_hash: fileHash,
    s3_key: s3Key,
    filename: filename,  // Send original filename to backend
  });
}
