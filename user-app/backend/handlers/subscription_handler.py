...
def get_user_plan_limits(user):
    ...
    try:
        ...
        print(f" Error in get_user_plan_limits: {str(e)}")
        # Fallback
    return {
            'plan': 'free',
            'plan_name': 'Free',
...
def handle_get_usage(user):
    """Get current usage and limits including video duration"""
    from utils.video_utils import get_user_video_usage
    ...
    video_quality = features.get('video_quality', 'none')
    
    video_usage = get_user_video_usage(user['id'], photos_table)
    video_used_minutes = video_usage['total_minutes']
    
    # Calculate video percentage
    if video_limit_minutes == -1:
        video_percent = 0  # Unlimited
        video_remaining_minutes = -1
    elif video_limit_minutes == 0:
        video_percent = 0
        video_remaining_minutes = 0
    else:
        video_percent = (video_used_minutes / video_limit_minutes * 100) if video_limit_minutes > 0 else 0
        video_remaining_minutes = max(0, video_limit_minutes - video_used_minutes)
    ...
    return create_response(200, {
        ...
        'video_limit': {
            'used_minutes': round(video_used_minutes, 2),
            'limit_minutes': video_limit_minutes,
            'usage_percent': round(video_percent, 2),
            'remaining_minutes': round(video_remaining_minutes, 2) if video_remaining_minutes != -1 else -1,
            'quality': video_quality,
            'video_count': video_usage['video_count']
        }
    })
...
def enforce_gallery_limit(user):
    ...
    if not gallery_limit['allowed']:
        limit = gallery_limit['limit']
        used = gallery_limit['used']
        plan_name = gallery_limit.get('plan', 'free')
        
        if plan_name == 'free':
            return False, f"Free plan allows only {limit} active galleries. You currently have {used}. Upgrade to Starter ($10/mo) for unlimited galleries."
        else:
            return False, f"You've reached your gallery limit ({limit}). Upgrade your plan for more galleries."
