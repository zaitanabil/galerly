...
from utils.email import send_gallery_expiration_reminder_email
    ...
    try:
        ...
        for gallery in all_galleries:
            ...
            if isinstance(expiration, (int, float)) or (isinstance(expiration, str) and expiration.isdigit()):
                ...
                if created_at:
                    try:
                        ...
                        print(f"⚠️  Error parsing expiration for gallery {gallery.get('id')}: {str(e)}")
                        continue
            ...
            # Handle expiration as ISO date string
                ...
                try:
                    ...
                    print(f"⚠️  Error parsing expiration date for gallery {gallery.get('id')}: {str(e)}")
                    continue
        ...
        for item in expiring_galleries:
            ...
            try:
                ...
                if not user_id:
                    print(f"⚠️  Gallery {gallery.get('id')} missing user_id")
                    continue
                ...
                if not users:
                    print(f"⚠️  User {user_id} not found for gallery {gallery.get('id')}")
                    continue
                ...
                if should_send_notification(user_id, 'gallery_expiring'):
                    ...
                    print(f"ℹ️  Skipping reminder for gallery {gallery.get('id')} - notifications disabled for photographer {user_id}")
