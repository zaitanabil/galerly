...
class StorageLifecycleManager:
    ...
    def __init__(self):
        self.s3_client = boto3.client('s3', region_name=os.environ.get('AWS_REGION'))
        self.bucket = os.environ.get('S3_PHOTOS_BUCKET')
        
        # Lifecycle policy configuration from environment (no fallbacks)
        # Days to keep originals in Standard before moving to Intelligent-Tiering
        self.original_standard_days = int(os.environ.get('LIFECYCLE_ORIGINAL_STANDARD_DAYS'))
        
        # Days to keep originals before archiving to Glacier
        self.original_glacier_days = int(os.environ.get('LIFECYCLE_ORIGINAL_GLACIER_DAYS'))
        
        # Days to keep renditions before moving to Infrequent Access
        self.rendition_ia_days = int(os.environ.get('LIFECYCLE_RENDITION_IA_DAYS'))
        
        # Days to keep temporary files before deletion
        self.temp_expiry_days = int(os.environ.get('LIFECYCLE_TEMP_EXPIRY_DAYS'))
        
        # Days to keep deleted content in Glacier before permanent deletion
        self.deleted_retention_days = int(os.environ.get('LIFECYCLE_DELETED_RETENTION_DAYS'))
        ...
        try:
            ...
            for storage_class in ['StandardStorage', 'IntelligentTieringStorage', 
                ...
                try:
                    ...
                    print(f" Could not get metrics for {storage_class}: {str(e)}")
