...
# Accept both 'development' and 'local' as local environments
...
def get_rendition_url(s3_key, size_name):
    ...
    if IS_LOCALSTACK_S3:
        ...
        from utils.config import S3_RENDITIONS_BUCKET
        # Replace 'localstack' hostname with 'localhost' for browser access
        endpoint = AWS_ENDPOINT_URL.replace('http://localstack:', 'http://localhost:') if AWS_ENDPOINT_URL else os.environ.get('AWS_ENDPOINT_URL')
