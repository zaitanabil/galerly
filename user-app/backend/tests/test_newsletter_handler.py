...
def mock_newsletter_dependencies():
    ...
    with patch('handlers.newsletter_handler.newsletters_table') as mock_newsletter:
...
class TestNewsletterSubscribe:
    ...
    def test_subscribe_success(self, mock_newsletter_dependencies):
        ...
        mock_newsletter_dependencies['newsletter'].get_item.return_value = {}
        
        body = {'email': 'subscriber@example.com', 'firstName': 'John'}
        result = handle_newsletter_subscribe(body)
        
        assert result['statusCode'] == 200
        response_body = json.loads(result['body'])
        assert 'subscribing' in response_body.get('message', '').lower() or 'success' in str(response_body).lower()
    ...
    def test_subscribe_duplicate_email(self, mock_newsletter_dependencies):
        ...
        mock_newsletter_dependencies['newsletter'].get_item.return_value = {
            'Item': {'email': 'subscriber@example.com', 'status': 'active'}
        }
        ...
        assert result['statusCode'] == 200
        response_body = json.loads(result['body'])
        assert 'already' in response_body.get('message', '').lower()
